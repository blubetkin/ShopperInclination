...
output:
  html_document:
    df_print: paged
---
## Importing of Deliverable 1 and libraries
Now, to start this next deliverable, we will import our previous work and continue!
```{r, Importing-deliverable1, message=FALSE, warning=FALSE}
...
train_model <- glm(Purchase ~ OperatingSystems + Browser + Shoppers_Region + TrafficType + VisitorType + Weekend, data = train)
predictions <- train_model %>% predict(test, type = 'response')
summary(train_model)
head(predictions)
```
...
train <- TheShoppers[sample_selection,]
test <- TheShoppers[-sample_selection,]
train_model <- glm(Purchase ~ OperatingSystems + Browser +  VisitorType + Weekend, data = train)
predictions <- train_model %>% predict(test, type ='response')
summary(train_model)
head(predictions)
```
...
train <- TheShoppers[sample_selection,]
test <- TheShoppers[-sample_selection,]
train_model <- glm(Purchase ~ Browser + VisitorType + Weekend, data = train)
predictions <- train_model %>% predict(test, type = 'response')
summary(train_model)
head(predictions)
```
